...
### gas-projects のアーカイブ化（2026-09-23、Claude）

- 利用者判断: 「もう使わない可能性が高いが、すぐには消したくない」
  → `gas-projects/` を `archive/gas-projects.zip` に圧縮し、元の
  ディレクトリは削除した。
- 圧縮前にファイル数（29件）・ファイル一覧の完全一致・`unzip -t`での
  破損無しを確認済み。
- `gas-projects/`は元々`.gitignore`対象（一度もコミットしていない）
  ため、このzipがローカルの唯一のコピーになる。`archive/`も
  `.gitignore`に追加済み（コミット対象外）。
- **万一zipも失っても、GASのスクリプトIDは本ファイルの冒頭（対象ファイル
  一覧）に記録済みなので、`clasp clone <スクリプトID>`でいつでも
  再取得可能**（元データはGoogle側にも残っている）。
